...
class Start {
  ...
  srcFile: string;
  constructor() {
    this.converter = new TsToLuaConverter(
      config.use_ts_basic_methods ? config.reserved_methods : {}
    ); // 创建转换器实例
  ...
  _init() {
    ...
    process
      .on("SIGTERM", exitHandler.bind(this))
      .on("SIGINT", exitHandler.bind(this)); // 优雅退出处理
  }
}
